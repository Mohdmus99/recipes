...
class _SearchScreenState extends State<SearchScreen> {
  ...
  //     _isLoading = true;
  //   });
  //   _recipes = (await _fetchData(pageNumber: _pageNumber, recipes: _recipes, searchText:_searchTextController.text))!;
  //   setState(() {
  //     _isLoading = false;
  //     _pageNumber++;
  //   });
  ...
  Future<void> _fetchData([String? searchText, String? action]) async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        if (searchText != null) {
          // _recipes = results.map((result) => Recipe(
          //   name: result['name'],
          //   rating: result['rating'],
          //   photoUrl: result['thumbnail_url'],
          // )).toList();
          ...
          _recipes.addAll(jsonRecipes
              .map((jsonRecipe) => Recipe.fromJson(jsonRecipe))
              .toList());
          // _recipes.addAll(results.map((result) => Recipe(
          //   name: result['name'],
          //   rating: result['rating'],
          //   photoUrl: result['thumbnail_url'],
          // )));
...
// Future<List<Recipe>?> _fetchData({String? searchText, String? tag, int? pageNumber, List<Recipe>? recipes}) async {
//
//   print(pageNumber);
...
//
//
//   String apiUrl = 'https://tasty.p.rapidapi.com/recipes/list?page=$pageNumber&size=20';
//   if (searchText != null) {
//     apiUrl += '&q=$searchText';
//   }
//
...
//     Uri.parse(apiUrl),
//     headers: {
//       'x-rapidapi-key': '6cbadfa0a6msh2a2b3c1493d3eb9p159dfbjsn74699e7acc60',
//       'x-rapidapi-host': 'tasty.p.rapidapi.com',
//     },
//   );
//
...
//         //   photoUrl: result['thumbnail_url'],
//         // )).toList();
//         recipes = jsonRecipes.map((jsonRecipe) => Recipe.fromJson(jsonRecipe)).toList();
//       } else {
//         recipes?.addAll(jsonRecipes.map((jsonRecipe) => Recipe.fromJson(jsonRecipe)).toList());
//         // _recipes.addAll(results.map((result) => Recipe(
//         //   name: result['name'],
//         //   rating: result['rating'],
//         //   photoUrl: result['thumbnail_url'],
